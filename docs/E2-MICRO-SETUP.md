...
- Reduced from 25 to 10 max connections (5 idle, see `backend/internal/config/postgres.go:22`)
...
```bash
# One-time check
./scripts/check-resources.sh   # Windows: scripts/check-resources.ps1

# Continuous monitoring (logs to monitoring.log)
./scripts/monitor.sh
```

> Windows hosts: `.ps1` equivalents exist for `check-resources`, `backup-db`, and `view-logs` (a `view-logs.sh` is also available).
